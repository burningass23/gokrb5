...
func TestUnmarshal_KerberosTokenV1(t *testing.T) {
	...
	expectedWrapToken := WrapToken {}
	expectedWrapToken.SGN_ALG    = kerToken[15:17]
	expectedWrapToken.SEAL_ALG   = kerToken[17:19]
	expectedWrapToken.SndSeqNum  = kerToken[21:29]
	expectedWrapToken.CheckSum   = kerToken[29:37]
	expectedWrapToken.Confounder = kerToken[37:45]
	expectedWrapToken.Payload    = kerToken[45:]

	var wt WrapToken
	err := wt.Unmarshal(kerToken, false)

	assert.Nil(t, err, "Unexpected error occurred.")
	assert.Equal(t, &expectedWrapToken, &wt, "Token not decoded as expected.")
}
...
func TestEncodeDecodeSndSeqNum(t *testing.T) {
	...
	WrapToken           := WrapToken {}
	WrapToken.SndSeqNum  = ssn

	WrapToken.encryptSndSeqNum(session, check)

	assert.Equal(t, expectedSndSeqNum, WrapToken.SndSeqNum, "SndSeqNum is not correctly encrypted")
}
