...
import (
	...
	"github.com/burningass23/gokrb5/v8/crypto"
	"github.com/burningass23/gokrb5/v8/iana/keyusage"
...
const (
	...
	HdrLen = 16
	// FillerByte is a filler in the WrapTokenV1 structure
	FillerByte byte = 0xFF
)

// WrapTokenV1 represents a GSS API Wrap token, as defined in RFC 4121.
...
func getGssWrapTokenV1Id() *[2]byte {
	return &[2]byte{0x05, 0x04}
}

// Marshal the WrapTokenV1 into a byte slice.
...
func (wt *WrapTokenV1) Marshal() ([]byte, error) {
	...
	copy(bytes[0:], getGssWrapTokenV1Id()[:])
	bytes[2] = wt.Flags
	bytes[3] = FillerByte
...
// the header, and sets the CheckSum field of this WrapTokenV1.
...
func (wt *WrapTokenV1) Unmarshal(b []byte, expectFromAcceptor bool) error {
	...
	if !bytes.Equal(getGssWrapTokenV1Id()[:], b[0:2]) {
		return fmt.Errorf("wrong Token ID. Expected %s, was %s",
			hex.EncodeToString(getGssWrapTokenV1Id()[:]),
...
// NewInitiatorWrapTokenV1 builds a new initiator token (acceptor flag will be set to 0) and computes the authenticated checksum.
